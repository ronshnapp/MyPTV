...
\fcolorbox{white}{bcolor}{
	\begin{minipage}{0.95\textwidth}
		\textbf{How segmentation is performed:} Segmentation in MyPTV is performed in one of two methods: \textit{labeling}, or \textit{dilation}. 
		...
		To imrove the results of segmenting over non-ideal images, MyPTV can be set to apply imae processing algorithms betfore the blob extraction: a gaussian blur filter, and local mean subtraction filter, and a mask. The first two filters mmay increase the computational time, however they may be necessary depending on the experimental conditions.
...
\begin{enumerate}
	...
	\item \texttt{particle\_size} - The particle size used in the dilation method.
\end{enumerate}
